...
public class requestpayload {
	...
	public static Object[][] FileNameCombine(String Filepath, String jsonpath,String requestValue,String nameOfAfile) throws FileNotFoundException {
        JsonElement jsonData = JsonParser.parseReader(new FileReader(Filepath));
        JsonElement dataSet = jsonData.getAsJsonObject().get(jsonpath);
        JsonArray jsonArray = dataSet.getAsJsonArray();
        ...
        for (int i = 0; i < jsonArray.size(); i++) {
            ...
            String jsonStr = jsonObject.toString();
            String templateName = jsonObject.get(requestValue).getAsString();
            String AddFileName = jsonObject.get(nameOfAfile).getAsString();
            //System.out.println(AddFileName);

            String FileName = templateName+"_"+AddFileName;
            result[i][0] = jsonStr;
            result[i][1] = FileName;
        }
...
//
	...
	@DataProvider(name = "getData")
	public Object[] dataproviders() throws FileNotFoundException {
		return requestpayload.getData1("ReportsFilesName.json", "dataSet", "templateName");
	}
